...
class SchemaValidationTest extends AbstractIntegrationTest {
    ...
    void testUserTableStructure() {
        ...
        String emailType = jdbcTemplate.queryForObject(
            """
            select data_type
            from information_schema.columns
            where table_schema = 'public'
    ...
    void testForeignKeyConstraints() {
        List<String> fkColumns = jdbcTemplate.queryForList(
            """
    ...
    void testUniqueConstraints() {
        Map<String, List<String>> uniqueConstraints = jdbcTemplate.query("""
        ...
        """, rs -> {
            ...
            while (rs.next()) {
                result
                    .computeIfAbsent(rs.getString("table_name"), key ->
