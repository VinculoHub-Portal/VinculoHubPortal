...
describe("ReportNpoModal", () => {
  ...
  it("não renderiza quando open é false", () => {
    render(
      <ReportNpoModal npoId={10} open={false} onClose={mocks.onCloseMock} />,
    )
  ...
  it("renderiza o formulário quando aberto", () => {
    ...
    expect(
      screen.getByRole("dialog", { name: "Denunciar ONG" }),
    ).toBeInTheDocument()
    expect(screen.getByLabelText(/Motivo da suspeita/)).toBeInTheDocument()
    expect(
      screen.getByRole("button", { name: "Limpar Campos" }),
    ).toBeInTheDocument()
    expect(
  ...
  it("fecha ao clicar no overlay fora do card", async () => {
    const user = userEvent.setup()
    render(<ReportNpoModal npoId={10} open onClose={mocks.onCloseMock} />)
  ...
  it("limpa o textarea e remove o erro", async () => {
    const user = userEvent.setup()
    render(<ReportNpoModal npoId={10} open onClose={mocks.onCloseMock} />)

    const reasonInput = screen.getByLabelText(/Motivo da suspeita/)

    await user.type(reasonInput, "curto")
    await user.click(screen.getByRole("button", { name: "Enviar denúncia" }))
    expect(
      screen.getByText("Descreva o motivo com pelo menos 10 caracteres."),
    ).toBeInTheDocument()

    await user.click(screen.getByRole("button", { name: "Limpar Campos" }))

    expect(reasonInput).toHaveValue("")
    expect(
      screen.queryByText("Descreva o motivo com pelo menos 10 caracteres."),
