...
export function CompanyShowcaseCard() {
  const [nameFilter, setNameFilter] = useState("")
  const [debouncedName, setDebouncedName] = useState("")

  const { companies, loading, error, currentPage, totalPages, totalElements, setCurrentPage, refetch } =
    usePaginatedCompanies({ pageSize: 10, name: debouncedName })
  ...
  return (
    <article className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-bold text-vinculo-dark">Empresas Cadastradas</h2>
          {!loading && !error && (
            <p className="mt-0.5 text-sm text-slate-500">
              {totalElements} {totalElements === 1 ? "empresa encontrada" : "empresas encontradas"}
            </p>
          )}
        </div>
      ...
      <div className={`transition-opacity duration-200 ${isFiltering ? "opacity-40" : "opacity-100"}`}>
        ...
        {!loading && !error && filteredCompanies.length === 0 && (
          <div className="flex h-80 flex-col items-center justify-center gap-2 text-slate-400">
            <ApartmentIcon sx={{ fontSize: 48, color: "#cbd5e1" }} aria-hidden />
            <p className="text-sm">
              {debouncedName
